"""Native cognis-connect emit for paywatch — forward findings to any platform.

Maps paywatch's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    paywatch ... --format json | paywatch-emit --to stix
    paywatch-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('merchant', None)
        out.pop('transaction_id', None)
        out.pop('date', None)
        out.pop('description', None)
        out['title'] = rec.get('merchant', 'Unknown Merchant')
        out['severity'] = 'medium'
        out['type'] = 'subscription'
        out['description'] = f"Recurring charge detected for {rec.get('merchant', 'Unknown Merchant')}"
        out['tags'] = ['subscription', 'recurring']
